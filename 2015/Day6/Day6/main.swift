...
//  Day6
//
//  Created by Stephen Darlington on 22/12/2015.
...
class Lights {
    ...
    func set(loc : Coord, value: Bool) {
        grid[loc.x + loc.y * self.y] = value
    }
    
    func get (loc : Coord) -> Bool {
        return grid [loc.x + loc.y * self.y]
    }
    
    func setRange (range : Range, operation : Operation) {
...
class Lights2 {
    ...
    func set(loc : Coord, operation: Operation) {
var value = get(loc)
switch operation {
case .On:
value = value + 1
...
grid [loc.x + loc.y * self.y] = value
    ...
    func setRange (range : Range, operation : Operation) {
        for x in range.start.x...range.end.x {
            for y in range.start.y...range.end.y {
                let c = Coord(x: x, y: y)
                set(c, operation:operation)
            }
        }
    }
    
    func lightsCount () -> Int {
        return grid.reduce(0, combine:+)
    }
}

//let x = Lights(x: 10,y: 10)
//x.lightsOn()
//x.set(Lights.Coord(x: 0,y: 0), value: true)
//x.lightsOn()
//x.setRange(Lights.Range(start: Lights.Coord(x: 0, y: 0), end: Lights.Coord(x: 9, y: 9)), operation: .On)
//x.lightsOn()
//x.setRange(Lights.Range(start: Lights.Coord(x: 1, y: 1), end: Lights.Coord(x: 2, y: 2)), operation: .Toggle)
//x.lightsOn()
...
let x = Lights2(x: 1000,y: 1000)

for instruction in input {
    let instructionComponents = instruction.componentsSeparatedByString(",")
    let operation = Operation(rawValue: instructionComponents[0])
    let start = Coord(x: Int(instructionComponents[1])!, y: Int(instructionComponents[2])!)
    let end = Coord(x: Int(instructionComponents[3])!, y: Int(instructionComponents[4])!)
    x.setRange(Range(start: start, end: end), operation: operation!)
}
