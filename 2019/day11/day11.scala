...
def nextDirection(d : Direction.Value, turn : Int) : Direction.Value = {
    d match {
        case Direction.U => if (turn == 0) { Direction.L } else { Direction.R }
        case Direction.R => if (turn == 0) { Direction.U } else { Direction.D }
        case Direction.D => if (turn == 0) { Direction.R } else { Direction.L }
        case Direction.L => if (turn == 0) { Direction.D } else { Direction.U }
    }
}

def nextLocation(x : Int, y : Int, dir : Direction.Value) : (Int,Int) = {
  dir match {
...
val robotMap = HashMap[(Int,Int),Int]()
var x, y = 0
var direction = Direction.U
var running = true
val startingColour = 0
var started = false
while (running) {
  val inputColour = if (robotMap.contains((x,y))) {
    robotMap((x,y))
  }
  else {
    if (!started) {
  ...
  vm.inputQueue.append(inputColour)
  val result = vm.runCalculation(true)
  if (result.size == 0) {
      val c = robotMap.size
      println (c)
      running = false
    ...
    val colour = result.head
    val turn = vm.runCalculation(true).head
    robotMap((x,y)) = colour.toInt
    val nextDir = nextDirection(direction, turn.toInt)
    direction = nextDir
    val n = nextLocation(x, y, nextDir)
    x = n._1
    y = n._2
  }
}
...
// Part 2
println ("Part 2")
val minX = robotMap.keys.map(_._1).min
val minY = robotMap.keys.map(_._2).min
val maxX = robotMap.keys.map(_._1).max
val maxY = robotMap.keys.map(_._2).max

for (y <- maxY to minY by -1) {
    for (x <- minX to maxX) {
      if (robotMap.contains((x,y))) {
          print ( if (robotMap((x,y)) == 0) { "⬛" } else { "⬜" } )
      }
      else {
          print ("⬛")
      }
