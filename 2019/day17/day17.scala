...
class VirtualMachine(memory : Buffer[Long]) {
  ...
  def runCalculation(stopOnOutput : Boolean = false) : Unit = {
  ...
  def outputToASCII : Seq[Char] = {
      val retv = outputQueue.map(_.toChar).toSeq
      outputQueue.clear
      retv
  }

  def printASCII : Unit = printASCII(outputToASCII)
  def printASCII(in : Seq[Char]) : Unit = {
      in.foreach(print(_))
  }
...
vm.runCalculation()
val output = vm.outputToASCII
vm.printASCII(output)

import scala.collection.mutable.HashSet

val scaffolding = new HashSet[(Int,Int)]()
...
output.foreach({
    ...
    case '\n' => x = 0
                 y += 1
    case '#' => scaffolding += ((x,y))
                x += 1
})

val intersections = scaffolding.map({
                               case (x,y) =>
                                   (x,y,scaffolding.contains((x-1,y)) &&
                                        scaffolding.contains((x+1,y)) &&
                                        scaffolding.contains((x,y-1)) &&
                                        scaffolding.contains((x,y+1)))
                               })
                            .filter({ case(x,y,z) => z == true})
                            .map({ case (x,y,z) => (x,y)})
                            .map({ case (x,y) => x * y })
                            .reduce(_+_)
